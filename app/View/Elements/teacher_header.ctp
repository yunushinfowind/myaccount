...
<div class="dashboard_header navbar navbar-default navbar-fixed-top">
    <div class="col-md-12">
		...
		if (!empty($teacherInfo['Teacher_information']['image'])) {
		    ?>
    		<img src="<?php echo $this->webroot; ?>img/teacher_images/<?php echo $teacherInfo['Teacher_information']['image']; ?>" class="img-circle" />
		<?php } else { ?>
    		<img src="<?php echo $this->webroot . 'img/default_user_icon.png'; ?>" class="img-circle" />
		<?php } ?>
            </i>
            <span class="dropdown">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php echo ucfirst($teacherInfo['User']['first_name']) . ' ' . ucfirst($teacherInfo['User']['last_name']); ?><span class="caret"></span>
                </a>
