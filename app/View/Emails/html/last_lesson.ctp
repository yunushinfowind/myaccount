...
<html>
    <head>
        ...
        <title><?php echo ucfirst($emailData['Email_content']['subject']); ?></title>
    </head>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <p style="padding:10px 25px 10px 25px; font-size:14px; margin:0px; background: #F4F4F4; border-bottom: 1px solid #d4d4d4;">
                                                <strong><?php echo ucfirst($emailData['Email_content']['subject']); ?></strong>
                                            </p>

                                            <?php
                                            $base_url = BASE_URL;
                                            $search = array('{name}', '{link}');
                                            $replace = array($user['User']['first_name'] . ' ' . $user['User']['last_name'], $base_url . 'student/make_payment');
                                            echo str_replace($search, $replace, $emailData['Email_content']['content']);
                                            ?>
                                        </td>
                                    </tr>
                                </table>
