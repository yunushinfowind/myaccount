...
<html>
    <head>
        ...
        <title><?php echo $email_data['Email_content']['subject']; ?></title>
    </head>
                                    ...
                                    <tr>
                                        <td colspan="2">
                                            <p style="padding:10px 25px 10px 25px; font-size:14px; margin:0px; background: #F4F4F4; border-bottom: 1px solid #d4d4d4;">
                                                <strong><?php echo $email_data['Email_content']['subject']; ?></strong>
                                            </p>

                                            <?php
                                            $search = array('{name}', '{amount}', '{pack_name}', '{transaction_id}', '{duration}', '{notes}');
                                            if (empty($paymentData['notes'])) {
                                                $replace = array(ucfirst($user_details['User']['first_name']) . ' ' . ucfirst($user_details['User']['last_name']), $user_details['Payment']['amount'], $pack_name, $paymentData['transaction_id'], $time);
                                            } elseif (!empty($paymentData['notes']) && isset($paymentData['notes'])) {
                                                if(!empty($paymentData['duration'])){
                                                    $duration = $paymentData['duration'];
                                                }else{
                                                    $duration = '0 minutes';
                                                }
                                                $replace = array(ucfirst($user_details['User']['first_name']) . ' ' . ucfirst($user_details['User']['last_name']), $user_details['Payment']['amount'], $pack_name, $paymentData['transaction_id'], $duration, 'Notes: "' . $paymentData['notes'] . '"');
                                            }
                                            echo str_replace($search, $replace, $email_data['Email_content']['content']);
                                            ?>
