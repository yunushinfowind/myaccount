        ...
        <div class="mess_page">
            ...
            if (!empty($all_message)) {
                foreach ($all_message as $message) {
                    ?>
                    <a href ="<?php echo BASE_URL . 'student/message_detail/' . $message['User']['id'].'#message'; ?>">
                        <div class="col-md-12">
                            <input type="hidden" value="<?php echo $studentinfo['User']['id']; ?>" class="currentUser">
                            <div class="messages <?php
                            if (($message['Message']['message_status'] == 'unread') && ($message['Message']['send_by'] != $studentinfo['User']['id'])) {
                                echo "bg_grey";
                            }
                            ?> clearfix">
                                 <?php
                                 if (!empty($message['Teacher_information']['image'])) {
                                     ?>
                                    <img src="<?php echo $this->webroot . 'img/teacher_images/' . $message['Teacher_information']['image']; ?>" class="img-circle" />
                                <?php } else { ?>
